// Public original 3f49040; independent measurement oracle.
interface FontAsset { family: string; url: string; unicodeRange?: string; ranges?: [number, number][] }
...
export async function prepareFonts(sources: string[]): Promise<void> {
  ...
  if (!faces.length) throw new Error('The pinned font oracle only supports Node measurements.');
